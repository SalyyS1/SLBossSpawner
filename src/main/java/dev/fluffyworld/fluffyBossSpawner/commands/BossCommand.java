package dev.fluffyworld.fluffyBossSpawner.commands;
...
public class BossCommand implements CommandExecutor, TabCompleter {
    ...
    public BossCommand(FluffyBossSpawner plugin, MessageManager messageManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            case "reload" -> {
                if (!sender.hasPermission("fluffyboss.reload")) {
            ...
            case "spawn" -> {
                if (!sender.hasPermission("fluffyboss.spawn")) {
                    sender.sendMessage(messageManager.getMessage("no-permission"));
                    return true;
                }
            ...
            case "despawn" -> {
                if (!sender.hasPermission("fluffyboss.despawn")) {
                    sender.sendMessage(messageManager.getMessage("no-permission"));
                    return true;
                }
            ...
            case "info" -> {
                if (!sender.hasPermission("fluffyboss.info")) {
                    sender.sendMessage(messageManager.getMessage("no-permission"));
                    return true;
                }
                sendInfo(sender);
    ...
    private void sendList(CommandSender sender) {
        ...
        for (Map.Entry<String, BossInstance> entry : instances.entrySet()) {
            ...
            String status = instance.isAlive() 
                ? messageManager.getMessage("status-alive") 
                : messageManager.getMessage("status-dead");
    ...
    private void sendInfo(CommandSender sender) {
        ...
        for (Map.Entry<String, BossInstance> entry : instances.entrySet()) {
            ...
            String expireTime = secondsUntilExpire >= 0 ? TimeUtils.formatDuration(secondsUntilExpire) : "N/A";
            placeholders.put("expire_time", expireTime);
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        if (args.length == 1) {
            ...
            List<String> subcommands = Arrays.asList("reload", "spawn", "despawn", "info", "list");
